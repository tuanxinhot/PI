...
JPIADMAPI_USER_CONF="/boot/jpiadmapi/user.conf"
JPIADMAPI_GROUP_CONF="/boot/jpiadmapi/group.conf"
OS_VERSION="`dumpe2fs -h $DEVICE_PART_2 2>&1 | grep 'Filesystem UUID' | cut -d: -f 2 | tr -d ' '`"
CURL_HEADER="Content-Type: application/json"
SHADOW="`cat /etc/shadow | sed '/systemd-coredump/d' | sed '/tcpdump/d' | md5sum | awk '{ print $1 }'`"
DEFAULT_SHADOW=""
FIRMWARE=""
KERNEL="`uname -r`"
...
if [ -f "$COMPOSE_YML" ]; then
  ...
  LAST_CHAR=$(echo $JSON_DATA | rev | cut -c1)
  if [ "$LAST_CHAR" != "[" ]; then
    JSON_DATA="${JSON_DATA::-1}"
  fi
  JSON_DATA="$JSON_DATA],"
...
### USER_CONF
if [ -f "$JPIADMAPI_USER_CONF" ]; then
  JSON_DATA="$JSON_DATA\"user_conf\":["
  while IFS='' read -r LINE; do
    JSON_DATA="$JSON_DATA\"`echo -n \"$LINE\" | tr -d "\r\n"`\","
  done < "$JPIADMAPI_USER_CONF"
  LAST_CHAR=$(echo $JSON_DATA | rev | cut -c1)
  if [ "$LAST_CHAR" != "[" ]; then
    JSON_DATA="${JSON_DATA::-1}"
  fi
...
### GROUP_CONF
if [ -f "$JPIADMAPI_GROUP_CONF" ]; then
  JSON_DATA="$JSON_DATA\"group_conf\":["
  while IFS='' read -r LINE; do
    JSON_DATA="$JSON_DATA\"`echo -n \"$LINE\" | tr -d "\r\n"`\","
  done < "$JPIADMAPI_GROUP_CONF"
  LAST_CHAR=$(echo $JSON_DATA | rev | cut -c1)
  if [ "$LAST_CHAR" != "[" ]; then
    JSON_DATA="${JSON_DATA::-1}"
  fi
